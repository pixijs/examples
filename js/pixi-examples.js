...
jQuery(document).ready(function($) {
    ...
    bpc.initNav = function() {
        ...
        $('.main-menu li').on(bpc.clickType, function() {
            if (!$(this).hasClass('selected')) {
                ...
                bpc.currentSource = 'examples/js/' + $(this).parent().attr('data-section') + '/' + $(this).attr('data-src');
    ...
    bpc.generateIFrameContent = function() {
        ...
        } else if (bpc.version.substr(0, 2) === 'v3') { // pull v3 from github cdn
        ...
        for (i = 0; i < plugins.length; i++) {
            html += '<script src="pixi-plugins/' + plugins[i] + '.js"></script>';
        }
