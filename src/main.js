import examplesData from '../public/examples/manifest.json';
...
jQuery(document).ready(($) => {
    ...
    bpc.init = function init() {
        ...
        const sections = Object.keys(examplesData);

        for (let i = 0; i < sections.length; i++) {
            if (examplesData[sections[i]].visible !== false) {
                let html = `<span class="section" data-section="${sections[i]}">${sections[i]}</span><ul data-section="${sections[i]}">`;
                const items = examplesData[sections[i]].items;
