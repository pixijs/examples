...
function testForHit( object1, object2 ) {
    ...
    var objectVertexList = object2.vertexData.reduce(
        function ( accumulator, vertex, index ) {
            if ( index % 2 === 0 ) {
...
function collisionResponse( object1Data, object2Data ) {
    var { object1, object1Acceleration, object1Mass } = object1Data;
    var { object2, object2Acceleration, object2Mass } = object2Data;

    if ( !object1 || !object2 ) {
        return;
    }

    var vCollision = new PIXI.Point(
        object2.x - object1.x,
        object2.y - object1.y
    );
    ...
    var vCollisionNorm = new PIXI.Point(
        vCollision.x / distance,
        vCollision.y / distance
    );
    var vRelativeVelocity = new PIXI.Point(
...
app.ticker.add( function ( delta ) {
    ...
    if ( app.screen.width > mouseCoords.x || mouseCoords.x > 0 ||
        ...
        var redSquareCenterPosition = new PIXI.Point(
        ...
        // the red square
        var toMouseDirection = new PIXI.Point(
            mouseCoords.x - redSquareCenterPosition.x,
            mouseCoords.y - redSquareCenterPosition.y
        );

        // Use the above to figure out the angle that direction has
        var angleToMouse = Math.atan2(
            toMouseDirection.y,
            toMouseDirection.x
        );
        ...
        var distMouseRedSquare = distanceBetweenTwoPoints(
            mouseCoords,
            redSquareCenterPosition
        );
        var redSpeed = distMouseRedSquare * movementSpeed;
